...
import { scrubEnv, SecureTempManager, SENSITIVE_PATTERNS } from '../../../src/infra/security/env-scrubber.js';
...
describe('Two-Phase Git Commit, Auto Lint/Test Loop & Env Scrubbing (P004)', () => {
  ...
  describe('2. Two-Phase Git Commit & Safe Rollback', () => {
    ...
    beforeEach(async () => {
      ...
      await execFileAsync('git', ['config', 'user.email', 'test@viharness.local'], { cwd: testRepoDir });
    ...
    afterEach(async () => {
      if (fs.existsSync(testRepoDir)) {
        try {
          ...
          fs.rmSync(testRepoDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
        } catch {
    ...
    it('should execute auto-lint after file write and inject failure as L0_HOT context', async () => {
      ...
      const compiled = await compiler.compile({
        ...
        targetModelDescriptor: {
          ...
          name: 'Mock',
          provider: 'mock',
          capabilities: { capabilities: [], maxContextTokens: 32000, maxOutputTokens: 4000, supportsToolCalling: true, supportsStreaming: false, supportsStructuredOutput: true, supportsVision: false },
        },
        budget: { maxTokens: 4000, softLimitTokens: 3500 },
        recentEvidence: iterationRecord.evidenceCreated,
      });

      const hotObject = compiled.retainedObjects.find((o) =>
        o.tags.includes('auto_feedback') && o.tier === ContextTier.L0_HOT,
      );
    ...
    it('should throttle auto-correction after reaching maxAutoCorrectionsPerFile', async () => {
      ...
      const mockVerificationEngine = {
        verify: async (target: any) => {
          ...
          return { status: VerificationStatus.PASSED, evidenceIds: [], taskId: task.id, verifiedAt: new Date(), durationMs: 0, confidence: 1.0, scope: 'repository' as const, affectedFiles: [], checkExecutions: [] };
        },
      };
