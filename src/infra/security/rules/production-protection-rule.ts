...
export class ProductionProtectionRule implements PolicyRule {
  ...
  public readonly description = 'Requires explicit human approval for actions in production environments or production-impacting changes.';
  ...
  async evaluate(action: PolicyAction, context?: PermissionContext): Promise<PolicyDecision> {
    ...
    if ((isProdEnv || isProdImpact) && !permContext.userApproved) {
      return {
        ...
        reason: 'Action targets production environment or contains production-impacting changes; human approval required.',
        ruleId: this.id,
        evaluatedAt: now,
        action,
      };
