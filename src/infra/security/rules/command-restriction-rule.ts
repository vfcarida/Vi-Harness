...
export class CommandRestrictionRule implements PolicyRule {
  ...
  public readonly description = 'Blocks forbidden shell commands and unsafe process execution vectors.';
  ...
  async evaluate(action: PolicyAction, context?: PermissionContext): Promise<PolicyDecision> {
    ...
    const isExec = actionType.includes('exec') || actionType.includes('cmd') || actionType.includes('shell') || actionType.includes('run');
