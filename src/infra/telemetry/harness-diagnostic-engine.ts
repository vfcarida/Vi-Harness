...
export class HarnessDiagnosticEngine {
  ...
  static diagnose(
    ...
    if (analysis.cacheHitRatio < 0.3 && analysis.promptTokens > 5000) {
      const rec: HarnessRecommendation = {
        ...
        confidence: 0.85,
        rationale: 'Invariant static prompt segments and repo-maps should be segregated with ephemeral cache headers to reduce latency and cost.',
    ...
    if (analysis.totalTokens > 80000 || analysis.totalIterations >= 15) {
      const rec: HarnessRecommendation = {
        ...
        actionablePatchSuggestion: 'Engage progressive 4-Stage Context Compaction with lower trigger threshold.',
      };
      recommendations.push(rec);
  ...
  static analyzeAcrossRuns(
    ...
    const toolFailureCounts = new Map<string, { count: number; total: number; evidence: string[] }>();
    ...
    for (const run of runs) {
      ...
      if (runTokens > 20000) {
        highTokenRunCount++;
        highTokenEvidence.push(`Run [${run.runId}]: Consumed ${runTokens} tokens for goal '${run.goalDescription}'`);
      }
    ...
    for (const [toolName, stats] of toolFailureCounts.entries()) {
      if (stats.count >= minRuns && stats.total > 0 && stats.count / stats.total >= 0.3) {
        ...
        const confidence = Math.min(0.95, 0.70 + (stats.count / runs.length) * 0.25);
    ...
    if (totalOscillationEvents >= minRuns || repairExhaustionCount >= minRuns) {
      ...
      const confidence = Math.min(0.95, 0.75 + (repairExhaustionCount / runs.length) * 0.20);
      if (confidence >= minConfidence) {
        ...
        recommendations.push({
          ...
          confidence,
          rationale: 'Frequent oscillation between implement and repair phases indicates reasoning breakdown during code generation. Dual-model Architect Mode decouples planning from editing to prevent cyclic repairs.',
          priority: 'HIGH',
    ...
    if (highTokenRunCount >= Math.max(1, Math.floor(runs.length * 0.4))) {
      ...
      if (confidence >= minConfidence) {
        recommendations.push({
          ...
          confidence,
          rationale: 'Long-horizon token accumulation frequently exceeds budget boundaries. Lower compaction trigger threshold to 0.65 to engage progressive compaction earlier.',
          priority: 'HIGH',
    ...
    if (lowCacheRunCount >= Math.max(1, Math.floor(runs.length * 0.4))) {
      ...
      if (confidence >= minConfidence) {
        recommendations.push({
          ...
          confidence,
          rationale: 'Cache hit ratios remain low across repeated runs. Segregate immutable system instructions and frozen memory snapshots with provider ephemeral cache headers.',
          priority: 'HIGH',
